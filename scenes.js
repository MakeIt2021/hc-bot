...
class SceneGenerator {
    ...
    GenActivityScene () {
        ...
        activity.enter(async (ctx) => {
            await ctx.reply('И последнее: как Вы можете оценить свою физическую активность', {
                reply_markup: {
                    inline_keyboard: [
                        [
                            {text: 'Вообще нет физических нагрузок', callback_data: '1.2'}
                        ],
                        [
                            {text: 'У меня лёгкая физическая нагрузка', callback_data: '1.375'}
                        ],
                        [
                            {text: 'У меня средняя физическая активность', callback_data: '1.55'}
                        ],
                        [
                            {text: 'У меня интенсивная физическая активность', callback_data: '1.725'}
                        ],
                        [
                            {text: 'У меня очень интенсивная физическая активность', callback_data: '1.9'}
                        ]
                    ]
                }
            })
        ...
        inTotal.enter(async (ctx) => {
            ...
            if (user.id === user_id && user_id === user_id_start) {
                ...
                inTotal.action('ok', async ctx => {
                    ...
                    // Мы проводим эти расчёты с использованием индекса массы тела — объективного коэффициента, описывающий связь между массой тела и ростом.
                    await ctx.reply('Этот бот может использовать данные, которые Вы передали только что, для расчёта индекса массы тела — объективного коэффициента, описывающего связь между массой тела и ростом. Хотя бот проводит все расчёты согласно рекомендациям ВОЗ, они могут оказаться неточными для тех, кому меньше 20 лет, и для других людей определённых категорий. Эти расчёты не могут быть использованы для постановки диагноза. \n\nПожалуйста, помните, что вы сами в ответе за своё здоровье. Более подробно Вы можете прочитать, отправив команду /disclaimer.')
                    await ctx.scene.enter('mainMenu')
                })
    ...
    GenMainMenuScene () {
        ...
        mainMenu.enter(async (ctx) => {
            await ctx.replyWithHTML(`<b>Основное<a href="https://telegra.ph/file/ca3893d892da3f23d0d06.png"> </a>меню:</b>`, {
                reply_markup: {
                    inline_keyboard: [
                        [
    ...
    GenWaterScene () {
        ...
        water.enter(async (ctx) => {
            ...
            await ctx.replyWithHTML(`<b>Сегодня Вы выпили ${str ? usersTempData.water : "0"} из ${water_recommended}</b> рекомендованных стаканов<a href="https://telegra.ph/file/d9397550a01cdecb1c304.png"> </a>воды (в одном стакане около 200 мл).`, {
                reply_markup: {
                    inline_keyboard: [
                        [
        ...
        meals.enter(async (ctx) => {
            ...
            if (await user) {
                ...
                if (bmi < 18.5) {
                    ...
                    result = `Мы посчитали Ваш ИМТ (индекс массы тела) по предоставленным Вами данным, он примерно равен ${bmi}. Это может указывать на недостаток веса. Задайте себе вопрос, часто ли бывает, что Вы чувствуете себя уставшим? \n` +
                        '\n' +
...
Мы посчитали, что Вам рекомендуется употреблять ~${calories} калорий в сутки для поддержания Вашего веса.` +
                    ...
                    result = `Мы посчитали Ваш ИМТ (индекс массы тела) по предоставленным Вами данным, он примерно равен ${bmi}. Это может указывать на наличие избыточной массы тела, предожирение. Это ещё не болезнь, но уже сигнал на то, что следует взять свой вес под контроль.\n` +
                        '\n' +
                    ...
                    result = `Мы посчитали Ваш ИМТ (индекс массы тела) по предоставленным Вами данным, он примерно равен ${bmi}. Это похоже на ожирение II степени. Пожалуйста, обратите внимание на информацию о том, как мы считали, и к кому относится приведённая здесь информация, отправив команду /about. С увеличением степени ожирения увеличивается риск развития сопутствующих заболеваний – от высокого до чрезвычайно высокого.
...
Лечение необходимо, т.к. часто из-за ожирения резко возрастают риски развития заболеваний сердечно-сосудистой, опорно-двигательной системы, некоторых онкологических заболеваний. Больше об этом можно узнать в статье всемирной организации здравоохранения.` +
                    ...
                    result = `Мы посчитали Ваш ИМТ (индекс массы тела) по предоставленным Вами данным, он примерно равен ${bmi}. Это похоже на ожирение III степени (максимальная степень ожирения). Пожалуйста, обратите внимание на информацию о том, как мы считали, и к кому относится приведённая здесь информация, отправив команду /about. С увеличением степени ожирения увеличивается риск развития сопутствующих заболеваний – от высокого до чрезвычайно высокого.
Мы бы очень рекомендовали Вам обратиться к врачу эндокринологу. Именно он может провести грамотное обследование и дать направления на возможные необходимые исследования. 
Лечение необходимо, т.к. часто из-за ожирения резко возрастают риски развития заболеваний сердечно-сосудистой, опорно-двигательной системы, некоторых онкологических заболеваний. Больше об этом можно узнать в статье всемирной организации здравоохранения.` +
                        '\n' +
                        '\n' +
    ...
    GenSportScene () {
        ...
        sport.enter(async (ctx) => {
            await ctx.replyWithHTML('"Жизнь требует движения",<a href="https://telegra.ph/file/dc2e4e804f1ce9c510c40.png"> </a>— говорил Аристотель. \n' +
                '\n' +
    ...
    GenStressScene () {
        ...
        stress.enter(async (ctx) => {
            await ctx.replyWithHTML('На протяжении всей<a href="https://telegra.ph/file/96d0512925363f911ebc5.png"> </a>жизни каждый человек сталкивается со стрессовыми ситуациями. Стресс — состояние психического напряжения, возникающее у человека под влиянием сильных воздействий.\n' +
                '\n' +
                ...
                '\n' +
                'Именно поэтому со стрессом важно справляться. Как только чувствуете эмоциональную нагрузку, поддержите себя, выполнив дыхательные упражнения с помощью кнопок ниже (кстати, они также эффективны перед сном). Оба упражнения займут не более 1.5 минут.', {
                reply_markup: {
                    inline_keyboard: [
                        [
    ...
    GenDisclaimerScene() {
        ...
        disclaimer.enter(async (ctx) => {
            await ctx.replyWithHTML('Бот предоставляет рекомендации, исходя из ИМТ, который представляет собой объективный коэффициент, описывающий связь между массой тела и ростом. ИМТ не учитывает состав тела, а значит не подходит для некоторых людей. По мнению экспертов ВОЗ, ИМТ не подходит людям с искусственно развитой мускулатурой (спортсмены-тяжеловесы, культуристы и пр.), лицам моложе 20 лет, беременным, кормящим матерям, при отечном синдроме и нарушениях водно-электролитного баланса.' +
                '\n' +
                '\n' +
                'Материалы данного ресурса, включая текст, графические элементы, изображения, ссылки на сторонние ресурсы и другие материалы, носят исключительно информационный характер. Материалы данного Сетевого ресурса не заменяют собой рекомендаций медицинского характера, диагностики или лечения, предоставляемых специалистами. Всегда обращайтесь к врачу по любым вопросам, которые могут возникнуть у вас в связи с состоянием здоровья. Если вы полагаете, что у вас возникла экстренная ситуация, немедленно обратитесь к своему врачу или позвоните по номеру экстренной службы 112. Health-coaching bot не рекомендует и не рекламирует никаких конкретных анализов, врачей, продуктов, процедур, а также не выступает в защиту каких-либо точек зрения или других сведений, упоминаемых в материалах данного ресурса. Ответственность за решения, принимаемые на основе каких-либо сведений, упоминаемых в материалах данного ресурса, — несете исключительно вы на ваш собственный риск.' +
                '\n' +
                '\n' +
                'Отправьте /start, чтобы вернуться к боту.'
            )
        })
        return disclaimer
    }
